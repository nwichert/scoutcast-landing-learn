...
export function SourceTrigger({ label, showFavicon = false, className }: SourceTriggerProps) {
    ...
    return (
        <HoverCardTrigger render={<a href={href} target="_blank" rel="noopener noreferrer" className={cn('bg-muted text-muted-foreground hover:bg-foreground/5 hover:text-primary inline-flex h-6 max-w-32 items-center gap-1 overflow-hidden rounded-full py-0 text-xs leading-none no-underline transition-colors duration-150', showFavicon ? 'pl-1 pr-2' : 'px-1', className)} />}>{showFavicon && (
                            <img
                                ...
                                className="size-3.5"
                            />
                        )}<span className="truncate text-center font-normal">{labelToShow}</span></HoverCardTrigger>
    )
}
